      ...
      <main id="main">
       {
        state.map((ele,ind)=>{
          return(
              <Cards key={ind} details={ele} setState={setState} state={state}/>
          )
        })
        
       }
        {(state.length==0)?(<div>No more tours<button className="btn" onClick={handleClick}>Refresh</button></div>):null}
